...
from sqlalchemy.dialects.mysql import INTEGER, SMALLINT, BOOLEAN, DECIMAL, VARCHAR, DATE, CHAR


class Funcionario(Base):
    __tablename__ = "funcionario"
    id: Mapped[int] = mapped_column(
        "id", Integer(), primary_key=True, unique=True, autoincrement=True)
    salario: Mapped[str] = mapped_column(
        "salario", DECIMAL(10, 2), nullable=False)
    carga_horaria: Mapped[int] = mapped_column(
        "carga_horaria", INTEGER, nullable=False)
